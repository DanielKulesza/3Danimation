...
const float ARM_FACTOR_MAX = 1.25;
const float ARM_FACTOR_MIN = 0.75;
const float MAX_LIGHT_ANGLE = 360;
const float MIN_LIGHT_ANGLE = 0;
...
void drawCylinder();
void outlineRenderCube();
void outlineRenderCylinder();
...
void updateKeyframeButton(int)
{
	...
	if (joint_ui_data->getID() > maxValidKeyframe)
		maxValidKeyframe = joint_ui_data->getID();

	// Update the appropriate entry in the 'keyframes' array
	// with the 'joint_ui_data' data
	keyframes[joint_ui_data->getID()].setDOFVector(joint_ui_data->getDOFVector());
	keyframes[joint_ui_data->getID()].setTime(joint_ui_data->getTime());
...
void initGlui()
{
	...
	glui_spinner = glui_joints->add_spinner_to_panel(glui_panel, "arm elongation factor:", GLUI_SPINNER_FLOAT, joint_ui_data->getDOFPtr(Keyframe::R_ARM_ELONG));
	glui_spinner->set_float_limits(ARM_FACTOR_MIN, ARM_FACTOR_MAX, GLUI_LIMIT_CLAMP);
	glui_spinner->set_speed(SPINNER_SPEED);
	...
	glui_spinner = glui_joints->add_spinner_to_panel(glui_panel, "arm elongation factor:", GLUI_SPINNER_FLOAT, joint_ui_data->getDOFPtr(Keyframe::L_ARM_ELONG));
	glui_spinner->set_float_limits(ARM_FACTOR_MIN, ARM_FACTOR_MAX, GLUI_LIMIT_CLAMP);
	glui_spinner->set_speed(SPINNER_SPEED);
	...
	//The light angle refers to the (cos(angle), sin(angle)) coordinates in the xy- plane some distance z away from the robot
	//
	glui_panel = glui_joints->add_panel("Light");

	glui_spinner = glui_joints->add_spinner_to_panel(glui_panel, "Position Angle", GLUI_SPINNER_FLOAT, joint_ui_data->getDOFPtr(Keyframe::LIGHT_POS));
	glui_spinner->set_float_limits(MIN_LIGHT_ANGLE ,MAX_LIGHT_ANGLE, GLUI_LIMIT_CLAMP);
	glui_spinner->set_speed(SPINNER_SPEED);
...
void display(void)
{
	...
	if (renderStyle == METALLIC){
		glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
		//The lights positiong is based on the angle of theta where theta is the angle with respect to the positive x axis. y =sintheta, x = costheta
		//lights position is 10 units in the z direction away. The camera rotates in a circle of radius 5
		GLfloat position[] = { 5 * cos(PI*joint_ui_data->getDOF(Keyframe::LIGHT_POS) / 180), 5 * sin(PI*joint_ui_data->getDOF(Keyframe::LIGHT_POS) / 180), 10, 1 };
		//Makes robot gold
		GLfloat colour[] = { 0.628281,	0.555802,	0.366065, 1 };
		GLfloat colour2[] = { 0.75164,0.60648	,0.22648, 1 };
		GLfloat colour3[] = { 0.24725, 0.1995, 0.0745, 1 };
		glLightfv(GL_LIGHT0, GL_POSITION, position);
		glMaterialfv(GL_FRONT, GL_AMBIENT, colour3);
	...
	if (renderStyle == MATTE){
		//Makes robot appear Matte
		glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
		GLfloat position[] = { 5 * cos(PI*joint_ui_data->getDOF(Keyframe::LIGHT_POS) / 180), 5 * sin(PI*joint_ui_data->getDOF(Keyframe::LIGHT_POS) / 180), 10, 1 };
		GLfloat colour[] = { 0.1, 0.1, 0.1, 1.0 };
		GLfloat colour2[] = { 0.0, 0.0, 0.0, 1 };
		GLfloat colour3[] = { 0.02, 0.02, 0.02, 1 };
	...
	glPushMatrix();
		// setup transformation for entire body
		...
		glRotatef(joint_ui_data->getDOF(Keyframe::ROOT_ROTATE_X), 1.0, 0, 0);
		glRotatef(joint_ui_data->getDOF(Keyframe::ROOT_ROTATE_Y), 0, 1.0, 0);
		glRotatef(joint_ui_data->getDOF(Keyframe::ROOT_ROTATE_Z), 0, 0, 1.0);
		...
		glPushMatrix();
				...
				drawCube();
				//called after every draw function in order to outline the objects
				outlineRenderCube();
		glPopMatrix();
		
		//DRAWING THE LEFT LEG*******************************************************************************************************************
		glPushMatrix();
		glTranslatef(-1.2, -1.6, 0);
		glRotatef(joint_ui_data->getDOF(Keyframe::R_HIP_PITCH), 0, 0, 1.0);
		glRotatef(joint_ui_data->getDOF(Keyframe::R_HIP_YAW), 0, 1.0, 0);
		glRotatef(joint_ui_data->getDOF(Keyframe::R_HIP_ROLL), 1.0, 0, 0);
		glScalef(0.25, 1.0, 0.25);
		glScalef(0.1/0.25, 0.1 , 0.1/0.25);
		glColor3f(1.0, 1.0, 1.0);
		drawCylinder();
		outlineRenderCylinder();
		glScalef( 0.25/0.1, 1.0/0.1, 0.25/0.1);
		...
		glPushMatrix();
		glTranslatef(1.2, -1.6, 0);
		glRotatef(joint_ui_data->getDOF(Keyframe::L_HIP_PITCH),0,0,1.0);
		glRotatef(joint_ui_data->getDOF(Keyframe::L_HIP_YAW), 0, 1.0, 0);
		glRotatef(joint_ui_data->getDOF(Keyframe::L_HIP_ROLL), 1.0, 0, 0);
		glScalef(0.25, 1.0, 0.25);
		glScalef(0.1 / 0.25, 0.1, 0.1 / 0.25);
		glColor3f(1.0, 1.0, 1.0);
		drawCylinder();
		...
		glPushMatrix();
		glTranslatef(-0.6, 0.20, 0);
		glRotatef(joint_ui_data->getDOF(Keyframe::R_SHOULDER_PITCH), 0, 0, 1.0);
		glRotatef(joint_ui_data->getDOF(Keyframe::R_SHOULDER_YAW), 0, 1.0, 0);
		glRotatef(joint_ui_data->getDOF(Keyframe::R_SHOULDER_ROLL), 1.0, 0, 0);
		glScalef(0.75, 0.25, 0.25);
		glScalef(0.1 / 0.75, 0.1/0.25, 0.1 / 0.25);
		glColor3f(1.0, 1.0, 1.0);
		drawCylinder();
		outlineRenderCylinder();
		glScalef(0.75 / 0.1, 0.25 / 0.1, 0.25 / 0.1);
		...
		//Scale back to (1,1,1) and positing at elbow
		glTranslatef(0.5,0,0);
			glScalef(1 / 0.75, 1 / 0.25, 1 / 0.25);
			glTranslatef(-1.0, 0, 0);
			glRotatef(joint_ui_data->getDOF(Keyframe::R_ELBOW), 0, 0, 1.0);
			glScalef(0.75*joint_ui_data->getDOF(Keyframe::R_ARM_ELONG), 0.20, 0.20);
			glScalef(0.1 / (0.75*joint_ui_data->getDOF(Keyframe::R_ARM_ELONG)), 0.1 / 0.20, 0.1 / 0.20);
		glColor3f(1.0, 1.0, 1.0);
		drawCylinder();
		outlineRenderCylinder();
		glScalef(0.75*joint_ui_data->getDOF(Keyframe::R_ARM_ELONG) / 0.1, 0.20 / 0.1, 0.20 / 0.1);
		glTranslatef(-0.5, 0, 0);
		glColor3f(0.0, 1.0, 0.0);
		drawCube();
		outlineRenderCube();
		...
		glPushMatrix();
		glTranslatef(0.6, 0.20, 0);
		glRotatef(joint_ui_data->getDOF(Keyframe::L_SHOULDER_PITCH), 0, 0, 1.0);
		glRotatef(joint_ui_data->getDOF(Keyframe::L_SHOULDER_YAW), 0, 1.0, 0);
		glRotatef(joint_ui_data->getDOF(Keyframe::L_SHOULDER_ROLL), 1.0, 0, 0);
		glScalef(0.75, 0.25, 0.25);
		glScalef(0.1 / 0.75, 0.1 / 0.25, 0.1 / 0.25);
		glColor3f(1.0, 1.0, 1.0);
		drawCylinder();
		...
		glScalef(1 / 0.75, 1 / 0.25, 1 / 0.25);
		glTranslatef(1.0, 0, 0);
		glRotatef(joint_ui_data->getDOF(Keyframe::L_ELBOW), 0, 0, 1.0);
		glScalef(0.75*joint_ui_data->getDOF(Keyframe::L_ARM_ELONG), 0.20, 0.20);
		glScalef(0.1 / (0.75*joint_ui_data->getDOF(Keyframe::L_ARM_ELONG)), 0.1 / 0.20, 0.1 / 0.20);
		glColor3f(1.0, 1.0, 1.0);
		drawCylinder();
		outlineRenderCylinder();
		glScalef(0.75*joint_ui_data->getDOF(Keyframe::L_ARM_ELONG) / 0.1, 0.20 / 0.1, 0.20 / 0.1);
		glTranslatef(0.5, 0, 0);
		...
		//DRAWING THE NECK/HEAD/EYES*******************************************************************************************************************
		glPushMatrix();
		glTranslatef(0, 0.9, 0);
		glRotatef(joint_ui_data->getDOF(Keyframe::HEAD_PITCH), 0, 0, 1.0);
		glRotatef(joint_ui_data->getDOF(Keyframe::HEAD_YAW), 0, 1.0, 0);
		glRotatef(joint_ui_data->getDOF(Keyframe::HEAD_ROLL), 1.0, 0, 0);
		glScalef(0.25, 0.75, 0.25);
		glScalef(0.1 / 0.25, 0.1 / 0.75, 0.1 / 0.25);
		glColor3f(1.0, 1.0, 1.0);
		drawCylinder();
		outlineRenderCylinder();
		glScalef(0.25 / 0.1, 0.75 / 0.1, 0.25 / 0.1);
		...
		glScalef(1 / 0.25, 1 / 0.75, 1 / 0.25);
		glTranslatef(0, 0.75 + 0.4, 0);
		glScalef(1, 0.4, 0.4);
		
		drawCube();
		outlineRenderCube();
		glScalef(1, 1/0.4,1/ 0.4);
...
//This function draws a 12 sided polygon that is supposed to mimc a cylinder
void drawCylinder(){
	glBegin(GL_QUADS);
	for (int i = 0; i <= 12; i++){
		
			glNormal3f(cos(i * 2 * PI / 12), sin(i * 2 * PI / 12), 0);	
			glVertex3f(cos(i*2*PI/12), sin(i*2*PI/12), 1.0);
			glNormal3f(cos(i * 2 * PI / 12), sin(i * 2 * PI / 12), 0);
			glVertex3f(cos(i * 2 * PI / 12), sin(i * 2 * PI / 12), -1.0);
			glNormal3f(cos((i+1) * 2 * PI / 12), sin((i+1) * 2 * PI / 12), 0);
			glVertex3f(cos((i+1) * 2 * PI / 12), sin((i+1) * 2 * PI / 12), -1.0);
			glNormal3f(cos((i + 1) * 2 * PI / 12), sin((i + 1) * 2 * PI / 12), 0);
			glVertex3f(cos((i+1) * 2 * PI / 12), sin((i+1) * 2 * PI / 12), 1.0);
	}
	glEnd();
}
//These functions check on every render whether an outline needs to be rendered as well
